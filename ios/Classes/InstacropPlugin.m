#import "InstacropPlugin.h"

#import <Photos/Photos.h>
#import <MobileCoreServices/MobileCoreServices.h>

@implementation InstacropPlugin
// + (void)registerWithRegistrar:(NSObject<FlutterPluginRegistrar>*)registrar {
//   FlutterMethodChannel* channel = [FlutterMethodChannel
//       methodChannelWithName:@"instacrop"
//             binaryMessenger:[registrar messenger]];
//   InstacropPlugin* instance = [[InstacropPlugin alloc] init];
...
+ (void)registerWithRegistrar:(NSObject<FlutterPluginRegistrar> *)registrar {
  FlutterMethodChannel *channel = [FlutterMethodChannel
      methodChannelWithName:@"plugins.com.aishayy/instacrop"
            binaryMessenger:[registrar messenger]];
  InstacropPlugin *instance = [InstacropPlugin new];
  [registrar addMethodCallDelegate:instance channel:channel];
}
...
- (void)cropImage:(NSString *)path
  ...
  [self execute:^{
    ...
    size_t scaledHeight =
        (size_t)(height * area.size.height * scale.floatValue);
    ...
    CGImageRef croppedImage = CGImageCreateWithImageInRect(
        image, CGRectMake(width * area.origin.x, height * area.origin.y,
    ...
    if (scale.floatValue != 1.0) {
      CGContextRef context = CGBitmapContextCreate(
          NULL, scaledWidth, scaledHeight, bitsPerComponent, bytesPerRow,
...
- (void)sampleImage:(NSString *)path
  ...
  [self execute:^{
    ...
    CGImageRef sampleImage =
        CGImageSourceCreateThumbnailAtIndex(image, 0, options);
    CFRelease(image);
...
- (bool)saveImage:(CGImageRef)image url:(NSURL *)url {
  CGImageDestinationRef destination =
      CGImageDestinationCreateWithURL((CFURLRef)url, kUTTypeJPEG, 1, NULL);
...
- (NSURL *)createTemporaryImageUrl {
  ...
  NSString *sampleName = [[@"instacrop_" stringByAppendingString:guid]
      stringByAppendingString:@".jpg"];
  NSString *samplePath =
      [temproraryDirectory stringByAppendingPathComponent:sampleName];
